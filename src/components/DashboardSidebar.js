...
export default function DashboardSidebar() {
  ...
  // Define a common dashboard link
  const commonDashboardLink = { name: 'Dashboard', href: '/dashboard', icon: <DashboardIcon fontSize="small" /> };
  ...
  useEffect(() => {
    // Get the specific links for the user's role
    const specificLinks = userInfo?.role ? roleBasedLinks[userInfo.role] || [] : [];
    
    // Prepend the common dashboard link to the specific links
    setActiveLinks([commonDashboardLink, ...specificLinks]);
  }, [userInfo]) // Dependency on userInfo to re-evaluate links when role changes
