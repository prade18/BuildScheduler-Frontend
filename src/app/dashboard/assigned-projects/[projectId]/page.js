...
//   const projectId = params.projectId; // This captures the [projectId] from the URL
...
//     fetchProjectData();
//   }, [projectId, router]); // Re-run effect if projectId or router changes

//   // Function to fetch main tasks with pagination
//   const fetchMainTasks = async (token, page, size) => {
//     setLoading(true); // Set loading while fetching tasks
//     try {
...
//       // Navigate to a new route for subtasks, e.g., /dashboard/assigned-projects/1/main-tasks/2/subtasks
//       router.push(`/dashboard/assigned-projects/${projectId}/main-tasks/${mainTaskId}/subtasks`);
//   };
...
//               <DetailItem label="Actual Start" value={project.actualStartDate || '-'} />
//               <DetailItem label="Actual End" value={project.actualEndDate || '-'} />
...
//                                 {/* Only "View Subtasks" button for Site Supervisor */}
//                                 <button 
...
export default function AssignedProjectDetails() {
  ...
  const [project, setProject] = useState(null);
  const [mainTasks, setMainTasks] = useState([]);
  const [loading, setLoading] = useState(true); // CORRECTED: Added useState(true)
  const [message, setMessage] = useState({ type: '', content: '' });
  ...
  // Function to redirect to subtasks page, using the recommended nested route
  const handleSubtaskRedirect = (mainTaskId) => {
      router.push(`/dashboard/assigned-projects/${projectId}/main-tasks/${mainTaskId}/subtasks`);
  };
